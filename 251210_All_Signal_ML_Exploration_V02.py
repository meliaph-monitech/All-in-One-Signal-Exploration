...
def pca_2d_plot(X: np.ndarray, meta: pd.DataFrame, color_col: str, title: str):
    ...
    if color_col in df_plot.columns:
        ...
        # fallback: color by defect label
        color_col = "defect_label"
        df_plot[color_col] = df_plot[color_col].astype(str)

    # Hover shows defect status + file + bead + color_col (cluster/status/etc.)
...
# Always read from session_state so tabs see latest data
        ...
        if len(selected_features) < 2:
            ...
            with model_tabs[0]:
                ...
                if st.button("Run K-Means"):
                    ...
                    # PCA plots with tooltips showing DEFECT STATUS etc.
